...
function ($scope, loginModal, advLoginModal, bootstrapModal) {
  ...
  function handleLoginSuccess(user) {
    $scope.loginMsgs.push({
      msg: 'User has been successfully authorized',
      data: user
    });
  }

  $scope.openLoginAdvModal = function () {
    $scope.loginMsgs.push({ msg: 'Requested Login Modal' });
    advLoginModal.open({
      onClose: function() { $scope.loginMsgs.push({ msg: 'Closed Login Modal' }); },
